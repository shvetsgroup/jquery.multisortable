			...
			if (!list_children.data('multiselectable')) {
				...
				list_children.click(function(e) {
					...
					if (!e.ctrlKey && !e.metaKey) {
						parent.find('.' + mySelectClass).removeClass(mySelectClass);
					} else {
						if (item.not('.child').length) {
							if (item.hasClass(mySelectClass)) {
								item.nextUntil(':not(.child)').removeClass(mySelectClass);
							} else {
								item.nextUntil(':not(.child)').addClass(mySelectClass);
						    }
						}
					}
					
					if (e.shiftKey && prevIndex >= 0) {
						prev.toggleClass(mySelectClass);
						if (prevIndex < myIndex) {
							item.prevUntil('.multiselectable-previous').toggleClass(mySelectClass);
						} else if (prevIndex > myIndex) {
							item.nextUntil('.multiselectable-previous').toggleClass(mySelectClass);
					    }
					}
					
					item.toggleClass(mySelectClass);
